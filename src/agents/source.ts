...
export interface PassthroughFile {
  /** POSIX path relative to `.agents/targets/claude/`. */
  relPath: string;
  content: string;
}
...
function loadClaudePassthrough(agentsDir: string): PassthroughFile[] {
  const claudeDir = path.join(agentsDir, "targets", "claude");
  if (!fs.existsSync(claudeDir)) return [];
  return listFilesRecursive(claudeDir)
    .sort()
    .map((relPath) => ({
      relPath,
      content: fs.readFileSync(path.join(claudeDir, relPath), "utf-8"),
    }));
}
...
export function loadSource(agentsDir: string): SourceModel {
  ...
  return {
    ...
    claudePassthrough: loadClaudePassthrough(agentsDir),
  };
}
