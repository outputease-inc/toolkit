...
function familiesFor(target: AgentTarget, source: SourceModel): Set<EmitterFamily> {
  ...
  if (target.id === "claude" && source.blocks.some((b) => b.frontmatter.paths !== undefined))
    families.add("claude-rules");
  if (target.id === "codex" && source.codexPassthrough.length > 0)
    families.add("codex-passthrough");
  if (target.id === "opencode" && source.opencodePassthrough.length > 0)
    families.add("opencode-passthrough");
  return families;
}
...
export function readManifestTargets(agentsDir: string): AgentTargetId[] | null {
  ...
  try {
    ...
    // A manifest that exists but does not parse (a merge conflict, a truncated write) must not
    // fall back to the phase default: that default still includes targets this project has
    // deliberately left (gemini since 2026-09-11), and regenerating would resurrect them.
    throw new Error(
      `${manifestPath} exists but does not parse (${err instanceof Error ? err.message : String(err)}). ` +
        "Resolve the conflict in the manifest first, or pass --targets explicitly.",
    );
  }
}
