 ...
 * the claude target it also lists the integration-owned passthrough skills and
 * the Claude-scoped asset classes. Deterministic (rows sorted) so the emitted
 ...
 * only the neutral source and the migrated passthrough tree are considered.
 */
...
function skillRow(skill: NeutralSkillFile, target: AgentTarget): FidelityRow {
...
/** Names of the integration-owned skills migrated as Claude passthrough. */
function passthroughSkillNames(source: SourceModel): string[] {
  const names = new Set<string>();
  for (const file of source.claudePassthrough) {
...
export function classifyFidelity(source: SourceModel, targets: AgentTarget[]): FidelityReport {
  ...
  for (const target of targets) {
    ...
    for (const skill of source.skills) rows.push(skillRow(skill, target));
    if (target.id === "claude") {
      for (const name of passthroughSkillNames(source)) {
        rows.push({
          item: name,
...
export function renderFidelityMd(report: FidelityReport, targets: AgentTarget[]): string {
  const lines: string[] = [
    "# Fidelity Report",
    "",
    "Per-target translation fidelity of the neutral source (spec 008, FR-011).",
    "Tiers: clean, degraded, skipped, claude-scoped, integration-owned, unplaceable.",
    "",
  ];
  for (const target of targets) {
