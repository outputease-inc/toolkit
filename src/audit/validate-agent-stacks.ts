...
import { ACTIVATABLE_CONDITIONS } from "../scaffold/agent-context";
...
export function validateCrossField(entries: AgentStackEntry[]): ValidationIssue[] {
  ...
  // Rule 24: marketplace ↔ installCommand consistency. Official-marketplace
  // plugins must not set marketplace; any other plugin marketplace suffix
  // requires it (the generator derives the `claude plugin marketplace add`
  // line from the field). MCP servers never carry a marketplace.
  for (const entry of entries) {
    if (entry.category !== "plugin") {
      if (entry.marketplace !== null) {
        issues.push({
          rule: "marketplace-installCommand",
          tool: entry.tool,
          details: `category is "${entry.category}" but marketplace is set (plugins only)`,
          severity: "error",
        });
      }
      continue;
    }
    const official = entry.installCommand.endsWith("@claude-plugins-official");
    if (official && entry.marketplace !== null) {
      issues.push({
        rule: "marketplace-installCommand",
        tool: entry.tool,
        details: "installCommand targets claude-plugins-official but marketplace is set",
        severity: "error",
      });
    }
    if (!official && entry.marketplace === null) {
      issues.push({
        ...
        tool: entry.tool,
        details:
          "installCommand targets a non-official marketplace but marketplace is null (needed for the marketplace-add line)",
        severity: "error",
      });
    }
  }

  // Rule 25: the condition must be one `resolveAgentStack` can switch on.
  // Anything else makes the entry unreachable: it passes every other rule,
  // reads as installable, and no project ever gets it. `playwright` was
  // auto-included with a working mcpConfig on `has_e2e_tests`, which no caller
  // produces, so it never reached a generated .mcp.json.
  //
  // Agent-stacks only. In dev-stacks `condition` is descriptive metadata across
  // 21 values that `resolveStack` never reads, so the sibling validator has no
  // equivalent rule and should not grow one.
  for (const entry of entries) {
    if (!ACTIVATABLE_CONDITIONS.includes(entry.condition)) {
      issues.push({
        rule: "condition-activatable",
        tool: entry.tool,
        details: `Condition "${entry.condition}" is never activated by resolveAgentStack, so this entry can never install. Activatable: ${ACTIVATABLE_CONDITIONS.join(", ")}`,
        severity: "error",
      });
    }
  }
