...
/**
 * Fetch + validate the registry metadata for the `@latest` dist-tag.
 * Shared by `fetchTarball` (needs dist.tarball) and `fetchLatestVersion`.
 */
async function fetchMetadata(metadataUrl: string, fetchImpl: FetchFn): Promise<RegistryMetadata> {
  let metaResponse: Response;
  try {
    metaResponse = await fetchImpl(metadataUrl, {
      redirect: "follow",
      headers: { accept: "application/json", "user-agent": "outputease-toolkit-update" },
    });
  } catch (err) {
...
/**
 * Fetch only the latest published version (metadata GET, no tarball download).
 * Used by `outputease upgrade` to decide whether the installed binary is stale.
 */
export async function fetchLatestVersion(
  metadataUrl: string = METADATA_URL,
  fetchImpl: FetchFn = fetch,
): Promise<{ version: string }> {
  const meta = await fetchMetadata(metadataUrl, fetchImpl);
