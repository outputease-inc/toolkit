...
export function renderSummary(
  ...
  const styledResult =
    summary.result === "success"
      ? role(resultText, "success", env)
      : summary.result === "error"
        ? role(resultText, "error", env)
        : role(resultText, "info", env);
  const lines = [
    paint("outputease update — summary", "lilac", env),
    "───────────────────────────",
    ...
    "",
    "Planned actions:",
    `  ${paint("+", "paleCyan", env)}  ${counts.added} added`,
    `  ~  ${counts.updatedClean} updated (clean)`,
    `  ${paint("!", "salmon", env)}  ${counts.overwritten} updated (overwritten — local edits replaced)`,
