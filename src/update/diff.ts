...
async function walkStaged(
  ...
  for (const name of entries) {
    ...
    // lstat (not stat) so a symlink is detected, not dereferenced. A malicious
    // upstream tarball could ship a symlink pointing at an out-of-tree host file
    // (~/.ssh/id_rsa, .env); dereferencing it would copy that file's content
    // into the project. Skip symlinks entirely, mirroring the other walkers.
