...
export const devStackEnvVarSchema = z
  .object({
    // Base name in SCREAMING_SNAKE_CASE with NO framework prefix baked in.
    // clientPrefix() applies NEXT_PUBLIC_/PUBLIC_/VITE_ at generation time.
    name: z
      .string()
      .min(1)
      .max(100)
      .regex(/^[A-Z][A-Z0-9_]*$/, "SCREAMING_SNAKE_CASE: uppercase, digits, underscore")
      .refine((n) => !/^(NEXT_PUBLIC_|PUBLIC_|VITE_)/.test(n), {
        message: "no framework prefix in the base name; clientExposed drives the prefix",
      }),
    purpose: z.string().min(1).max(200),
    whereToGet: z.string().max(300).nullable(),
...
export const devStackEntrySchema = z.object({
  ...
  env: z
    .array(devStackEnvVarSchema)
    .min(1)
    .refine((vars) => new Set(vars.map((v) => v.name)).size === vars.length, {
      message: "duplicate env var names within one tool",
    })
    .optional(),
});
...
export type DevStackEnvVar = z.infer<typeof devStackEnvVarSchema>;
