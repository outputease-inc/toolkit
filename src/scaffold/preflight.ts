...
/**
 * Build an env object with the current PATH, normalized to a single `PATH`
 * key. Mirrors `cloneEnvWithoutPath` in post-install.ts (Windows is case-
 * insensitive at the OS level but Node may expose both `Path` and `PATH`).
 */
