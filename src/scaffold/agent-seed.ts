...
function copyDir(sourceDir: string, destDir: string, skip?: (rel: string) => boolean): void {
...
function partitionClaudeTemplateTree(
  ...
  for (const rel of listFiles(claudeTemplateDir)) {
    ...
    // Design §5.8: `speckit-archive` is OE-authored so it survives the payload gate,
    // but it references `.specify/` and `archive/specs/` (SKILL.md:14) and would
    // dangle on the `--no-speckit` path. Emit spec-kit-shaped skills only when the
    // project actually gets spec-kit.
    if (!specKit && rel.startsWith("skills/speckit-")) continue;
...
function copyCodexTargetTemplates(
  agentsDir: string,
  templatesRoot: string = TEMPLATES_DIR,
  specKit = true,
): void {
  ...
  copyDir(
    sourceDir,
    path.join(agentsDir, "targets", "codex"),
    (rel) => !specKit && rel.startsWith("skills/speckit-"),
  );
}
...
export function materializeToolkitSource(
  ...
  opts: {
    ...
    /** Emit `speckit-*` skills? Defaults true — the update path (src/cli/update.ts:171) keeps today's behavior. */
    specKit?: boolean;
  },
): void {
  const templatesRoot = opts.templatesRoot ?? TEMPLATES_DIR;
  const specKit = opts.specKit ?? true;
  partitionClaudeTemplateTree(agentsDir, templatesRoot, specKit);
  copyCodexTargetTemplates(agentsDir, templatesRoot, specKit);
...
export function buildAgentFiles(opts: AgentSeedOptions): AgentSeedResult {
  ...
  try {
    ...
    materializeToolkitSource(agentsDir, {
      ...
      specKit: opts.templateData.specKit,
    });
