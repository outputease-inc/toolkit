...
/**
 * Every condition `activateConditions` below can switch on, and therefore the
 * only conditions a dataset entry may carry: an entry conditioned on anything
 * else can never install, whatever its `selectionMode` says. `playwright` sat
 * at `auto-included` with a working `mcpConfig` and `condition:
 * "has_e2e_tests"`, which nothing here produces, so it never once reached a
 * generated `.mcp.json`. `validate-agent-stacks.ts` enforces this per entry.
 *
 * This is an agent-stacks invariant only. In dev-stacks `condition` is
 * descriptive metadata across 21 values that `resolveStack` never reads, so the
 * same rule there would be wrong on the merits.
 */
export const ACTIVATABLE_CONDITIONS: readonly string[] = [
  "always",
  "optional",
  "has_frontend",
  "backend:supabase",
...
/** The conditions one project shape switches on. Union over all shapes === ACTIVATABLE_CONDITIONS. */
function activateConditions(platformKey: string, backend?: string): Set<string> {
  // `optional` is not an exclusion mechanism: it passes here for every project,
  // and `selectionMode: "selectable"` is what keeps those entries out of the
  // automatic install.
  const active = new Set<string>(["always", "optional"]);
  if (platformKey !== "tooling") {
    active.add("has_frontend");
  }
  if (backend === "supabase") {
    active.add("backend:supabase");
  }
  return active;
}
...
export function resolveAgentStack(platformKey: string, backend?: string): AgentStackEntry[] {
  ...
  const activeConditions = activateConditions(platformKey, backend);
