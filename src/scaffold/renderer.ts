...
function collectTemplates(
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      ...
      let content = eta.render(templatePath, data);
      // package.json is assembled from partials + `forEach` loops, and eta's
      // newline handling (autoTrim) collapses the entries onto single lines and
      // fuses closing braces — valid JSON, but mangled whitespace. Normalize any
      // rendered package.json to canonical 2-space JSON. Guarded: a non-JSON or
      // JSONC render falls back to the raw output rather than breaking the scaffold.
      if (outputPath === "package.json" || outputPath.endsWith("/package.json")) {
        try {
          content = `${JSON.stringify(JSON.parse(content), null, 2)}\n`;
        } catch {
          // leave the raw render untouched
        }
      }
