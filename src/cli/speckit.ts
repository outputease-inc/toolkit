...
export async function speckitAction(
  ...
  // Per-site overlay outcomes. This is the ONLY enumeration of OutputEase's spec-kit
  // customization sites an operator ever needs: the instruction block deliberately does
  // not restate them, because a second list is a hand-maintained fact that goes stale
  // (Principle VI). It is derived from OVERLAY_RULES on every run, so a rule added or
  // removed shows up here without anyone updating prose.
  if (result.overlay.length > 0) {
    log.info(
      [`Customization sites (${result.overlay.length}):`]
        .concat(result.overlay.map((o) => `  ${o.outcome.padEnd(13)} ${o.file}`))
        .join("\n"),
    );
  }

  if (result.refreshedSkills.length > 0) {
    log.success(
      `Advanced ${result.refreshedSkills.length} spec-kit skill(s) in .agents/targets/claude/.`,
    );
  }
