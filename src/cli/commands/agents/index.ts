...
import { check, formatDrift } from "../../../agents/check";
...
export function makeAgentsCommand(): Command {
  ...
  agents
    ...
    .action(() => {
      ...
      if (result.exitCode === 0) {
        console.log(pc.green("✓ no drift — generated tree matches the neutral source"));
      } else if (result.exitCode === 2) {
        for (const err of result.errors) console.error(pc.red(err));
      } else {
        console.error(pc.red(`✗ drift detected (${result.drift.length}):`));
        for (const line of result.drift) console.error(`  ${formatDrift(line)}`);
        console.error(pc.dim("  fix the neutral source and run: bun run agents:generate"));
      }
      process.exit(result.exitCode);
    });
