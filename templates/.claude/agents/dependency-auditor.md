...
1. Determine the **workspace scope** from the root `package.json` `name` (or any workspace member), e.g. `@outputease`. All checks below use this derived scope rather than a hardcoded one.
...
All internal workspace-scoped dependencies (the scope derived above, e.g. `@outputease/*`) must use `workspace:*`.
