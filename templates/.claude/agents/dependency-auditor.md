...
1. Discover all `package.json` files across `packages/` and `apps/`
2. Read each one and build a dependency map
3. Run through each audit check below
...
All internal `@outputease/*` dependencies must use `workspace:*`.

- Read each `package.json`
- For every dependency in `dependencies`, `devDependencies`, and `peerDependencies`:
  - If the package name starts with `@outputease/`, verify the version is exactly `workspace:*`
...
- Build the full dependency graph: which `@outputease/*` packages depend on which
- Flag any package that has zero inbound edges
- Exclude packages consumed via non-npm mechanisms:
  - `config-typescript` -- consumed via `tsconfig.json` `extends`
  - `config-tailwind` -- consumed via CSS `@import`
  - `brand` -- consumed via CSS `@import` through config-tailwind
  - `toolkit` -- standalone CLI tool
...
3. Build the internal package registry: all `@outputease/*` names
4. Run all 4 audit checks
...
- **coderabbit code review** -- Feeds dependency audit findings into PR review when `package.json` files are modified
