---
name: checkpoint
description: WIP commit with lite handoff - updates HANDOFF.md/TODO.md for file and external session-state changes, skips pre-commit hooks, no push
...
Before exiting on a clean worktree, review the current conversation/tool actions
against `HANDOFF.md` and `TODO.md`. A checkpoint must capture meaningful
...
`TODO.md` first, then continue with the checkpoint. If interactive questioning is
unavailable, make conservative non-destructive updates that preserve open work
and record only facts established in the session.
...
git grep --cached -lE "sk-|pk_live|PRIVATE_KEY=" -- . 2>/dev/null | head -3
```
...
2. **Update "Last Session" header**: in the block between the `# Next Steps` title and the first `##` heading, REPLACE the `**Last Session**:` line with a single new one and delete every other narrative line in that block — never retain or demote the previous entry: no `<!-- prior: -->` markers, no `**Prior Session**` lines. The Recent Session Log table is the only in-file history; git preserves the rest. RETAIN the `**Current Focus**` line (update it if the work area changed; add it on line 4 if missing). Net result: the block contains exactly two content lines — `**Last Session**` and `**Current Focus**`.
   - Format: `**Last Session**: YYYY-MM-DD ([work area from changed files or session-state updates])`
   - Infer work area from changed file paths or, when the checkpoint is documenting external/session state, from the affected project area:
     - `packages/*/` -> Package name
     - `apps/*/` -> App name
     - `docs/` -> Documentation
     - `.claude/` -> Claude configuration
     - `specs/` -> Feature specification
     - external project-state updates only -> Documentation
...
3. **Add session log entry** to the "Recent Session Log" table:
   - **Date**: Today's date
   - **Work Completed**: Brief summary from changed files plus explicit session/tool actions that changed project state
   - **Next Action**: Infer from current feature/directory and any updated external blockers or follow-ups
...
### 4.1. Sync TODO.md Progress Notes

Read `TODO.md` if it exists. Under `## Ideas Backlog`, update active entries
when this session materially changed their state, even if no code changed:

- Add or revise concise progress/status notes for vendor answers, sent support emails, purchases, setup state, research conclusions, blockers, and next actions.
- Keep entries on the backlog unless the user explicitly completed or removed the idea; checkpoint is non-destructive.
- Do not append change narrative to the footer. If `TODO.md` changes, set the footer exactly to `**Last Updated**: YYYY-MM-DD`.
- If no active entry is affected, leave `TODO.md` unchanged.
