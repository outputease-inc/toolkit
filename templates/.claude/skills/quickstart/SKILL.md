...
### 2.5.3 Delegate to /develop-idea (mandatory)

If the user selected a backlog idea (anything other than "Skip for now"), this
is a **BLOCKING REQUIREMENT**: invoke `/develop-idea [idea-name]` via the Skill
tool BEFORE any other action. Do NOT run an inline clarification loop here, and
do NOT continue to Phase 3 until `/develop-idea` has run. Refinement, the
readiness check, the idea brief, and any spec/direct handoff are owned entirely
by `/develop-idea` — quickstart does not duplicate that logic.
...
- If the user picked an idea in Phase 2.5: `/develop-idea` already ran (mandatory
  delegation, step 2.5.3). Its outcome governs what happens next — `[Ready: Spec]`
  hands off to `/speckit-specify` (session ends here), `[Ready: Direct]` begins
  implementation in the current branch, anything else continues to the Final Summary.
