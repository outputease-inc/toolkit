...
- The `packages/types/src/index.ts` currently contains `ApiResponseSchema` inline. As domain schemas are added, it transitions to a re-export hub. The inline `import { z }` can be removed once all schemas live in domain files.
