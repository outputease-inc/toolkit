...
(This is the allow-listed form per `CLAUDE.md § Pushing to main`; bare `git push origin HEAD` is not on the settings.json allow list and prompts.)
...
### 7.5. Notion PM Status Sync (feature 007)

**Only if `apps/web/scripts/notion-sync.ts` exists** (the OutputEase web app's
Notion status-sync runner). If it does not exist, skip this step silently — this
keeps the step harmless in any project scaffolded from the toolkit template.

After the commit + push, reflect any repo-declared Epic/Milestone status up to
Notion (one-way, status-only, idempotent, never creates records). Always preview
first, then apply:

```bash
cd apps/web
bun scripts/notion-sync.ts            # preview — lists every intended change (from -> to), writes nothing
bun scripts/notion-sync.ts --apply    # writes ONLY the Status property of mapped records
cd - >/dev/null
```

- Declarations live in `notion-sync.json` at the repo root; declaring a status is
  a one-line edit to a `declaredStatus` value (the six values: `Backlog`,
  `Planned`, `In Progress`, `Blocked`, `Done`, `Cancelled`).
- Verify the preview `from -> to` lines match intent before running `--apply`.
- Exit `2` = aborted in preconditions (bad manifest / missing token) with zero
  writes; `1` = completed with >=1 failed record (others still applied); `0` = clean.
- `/checkpoint` deliberately does NOT run this — frequent WIP states stay off the
  client-visible surface.

### 7.6. Queued-Release Reminder
