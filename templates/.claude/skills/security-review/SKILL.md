...
- For a whole-repository sweep rather than a surface pass — an unattended
  multi-agent scan with adversarial verification — hand off to
  `claude-security` if it is installed. It is the escalation, not the
  default: this skill's passes are scoped to the surfaces you name and
  finish in one session, which is the right shape for a pre-deploy check.
...
- **claude-security** plugin (if installed): whole-repository unattended scan; the escalation from this skill's scoped passes
...
- **`/code-review <PR#>`** on the opened PR; a GitHub review App, where a project has one, adds a pass but is not the gate
