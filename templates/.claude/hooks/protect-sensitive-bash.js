...
process.stdin.on('end', () => {
  ...
  try {
    ...
    //
    // The `.env` pattern is anchored on its LEFT edge (spec 010, finding H5). A `.env`
    // file is preceded by nothing, a path separator, whitespace, a quote, or an `=`/`:`
    ...
    // any pipeline that printed an environment variable and also touched a file, while
    // touching no secret at all. Anchoring is the right layer: the command really does
    // read a file, just not that one, so loosening the reader stage above would have
    // widened the hole instead of closing the false positive.
