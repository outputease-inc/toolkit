 ...
 * Runs `biome format --write` on the edited file. NOT `biome check` — per the
 * "Biome check unsafe in hooks" gotcha, `check` builds the full workspace module
 * graph even for one file and is far too slow for a hook (`format` is ~6ms).
 *
 * Exit codes:
...
const MAX_INPUT = 1024 * 1024; // 1 MB: a whole-file Write exceeds 10 KB on the wire
...
process.stdin.on("end", () => {
  ...
  try {
    ...
    // Run `biome format --write` on the file (format only — see header).
