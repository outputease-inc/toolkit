...
process.stdin.on("end", () => {
  ...
  try {
    ...
    if (/^\.env([._-]|$)/i.test(fileName) && !/\.(example|sample|template)$/i.test(fileName)) {
    ...
    if (
      /\.(tfstate|tfstate\.backup|crt|cer)(\.(bak|backup|old|orig|save|tmp|swp|\d+))?$/i.test(fileName) ||
      /\/\.(aws|gcp|azure|docker|kube)\//i.test(normalized)
    ) {
      process.stderr.write(
    ...
    if (
      /\.(pem|key|pfx|p12|secret|token|keystore|jks|gpg|asc)(\.(bak|backup|old|orig|save|tmp|swp|\d+))?$/i.test(fileName) ||
      /^(credentials\.json|\.npmrc|\.pypirc|\.htpasswd|\.netrc|id_rsa|id_ed25519|id_dsa|id_ecdsa|authorized_keys|known_hosts)(\.(bak|backup|old|orig|save|tmp|swp|\d+))?$/i.test(
        fileName,
      )
    ) {
      process.stderr.write(
    ...
    if (/^secrets\.(yaml|yml|json)$/i.test(fileName) || /^vault\.(json|yaml|yml)$/i.test(fileName)) {
      process.stderr.write(
    ...
    if (/\/\.(ssh|credentials)\//i.test(normalized) || /\/secrets\//i.test(normalized)) {
      process.stderr.write(
