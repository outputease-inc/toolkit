...
- **Built-in vs plugin.** `/code-review` and `/simplify` are **built-in** Claude Code commands. The
  `code-review` and `code-simplifier` *plugins* duplicate them and are prune candidates — prefer the
  built-ins.
