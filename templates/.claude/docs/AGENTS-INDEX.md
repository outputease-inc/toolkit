...
- **`/code-review <PR#>`** — the built-in review run on the pull request before it merges; no GitHub
  review App is assumed.
...
> reaches a PR review. Commits themselves go through `/commit`, the one
...
| **package.json changes** | dependency-auditor | `/code-review <PR#>` |
...
+-- Ready for PR?
    YES --> superpowers:requesting-code-review (orchestrates local agents)
         + /code-review <PR#> once the PR is opened
...
**Chain**: `test-writer` --> `accessibility-reviewer` --> open PR --> `/code-review <PR#>`

1. **test-writer**: Generate unit and accessibility tests
2. **accessibility-reviewer**: Validate WCAG 2.1 AA compliance
3. **Push and open PR**: run `/code-review <PR#>` on it before asking to merge

### Security Concern

**Chain**: `/security-review` skill --> `test-writer` --> `/code-review <PR#>`
...
3. **`/code-review <PR#>`**: review the PR once it opens
...
5. **`/code-review <PR#>`** (on PR open): bugs, security and simplification findings on the PR diff

### Dependency Health Check

**Chain**: `dependency-auditor` --> `/code-review <PR#>`

1. **dependency-auditor**: Audit workspace protocol, version conflicts, orphans, peer deps
2. **`/code-review <PR#>`**: reviews the change on the PR
...
> The 4 local agents (`accessibility-reviewer`, `test-writer`, `i18n-reviewer`,
> `dependency-auditor`) occupy narrow domains that no plugin this
> repository *depends on* covers — those declared in `.agents/plugins.json`. It previously
