...
- `.specify/` contains the spec-kit workflow assets for multi-step feature work, when spec-kit is installed (`outputease speckit init`).
...
4. Continue from the project handoff and git state after interruptions.
...
- `continue`: recover state after a pause or crash.
...
- Run slash commands such as `/quickstart`, `/checkpoint`, `/continue`, `/dev-check`, and `/session-end` when those commands are present.
...
When spec-kit is installed (`outputease speckit init`), use `.specify/` for larger features, multi-session work, or tasks that need structured planning and tracking.
...
Agent wrappers may expose these steps differently, but when spec-kit is installed the lifecycle and source files live under `.specify/`.
...
- [AGENTS.md](../AGENTS.md)
- [.agents/](../.agents/)
- `.specify/` (present only when spec-kit is installed: `outputease speckit init`)
